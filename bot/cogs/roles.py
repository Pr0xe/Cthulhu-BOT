...
class Roles(commands.Cog):
    ...
    async def crole_error(self, ctx ,error):
        if isinstance(error, commands.MissingPermissions):
            embed=discord.Embed(title="Permission Denied.", description=f"{ctx.message.author.mention} No permission to use this command.", color=0xff0000)
            await ctx.send(embed=embed)
        elif isinstance(error, commands.MissingRequiredArgument):
            embed=discord.Embed(title="Input ERROR", description=f"{ctx.message.author.mention} Arguments are missing", color=0xff0000)
            await ctx.send(embed=embed)
    ...
    async def addrole_error(self, ctx ,error):
        ...
        #Bad syntax for roles
            ...
            embed=discord.Embed(title="Input ERROR", description=f"{ctx.message.author.mention} Bad syntax", color=0xff0000)
            await ctx.send(embed=embed)
        #No role
    ...
    async def rmrole(self, ctx, role: discord.Role, user: discord.Member):
        if role not in user.roles:
            embed=discord.Embed(title="Remove Role Error", description=f"{user.mention} is not {role.mention}", color=0xff0000)
            await ctx.send(embed=embed)
        else:
